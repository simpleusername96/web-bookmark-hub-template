...
- `docs/images/{reading,summary,library,organize,search}-{ko,en}.png` and `docs/images/extension-{connect,save,saved}.png` show the unchanged production app and native extension in a fresh Chrome profile with isolated dummy data. Saving, tag assignment, Folder moves and search use real UI controls; personal bookmarks, browsing history and signed-in accounts are absent. The extension pairing and URL save are real; summaries and initial folder/tag assignments are authored fixtures, and the blog's cover is generated artwork. Cards use representative images or browser screenshots of invented source pages, stored through the real Registry as page-snapshot covers; the summaries remain stored alongside them. No live model call or named-platform compatibility test is represented. This does not authorize publishing user collections or runtime captures.
